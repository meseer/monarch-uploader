...
export function createAuth(storage: StorageAdapter) {
  ...
  return {
    setupMonitoring(): void {
      ...
      page.fetch = async function interceptedFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
      ...
      OriginalXHR.prototype.open = function interceptedOpen(method: string, url: string | URL, ...args: unknown[]): void {
      ...
      OriginalXHR.prototype.setRequestHeader = function interceptedSetRequestHeader(name: string, value: string): void {
        const request = requests.get(this);
